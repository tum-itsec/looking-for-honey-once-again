    ...
    def format(self, data, depth):
        ...
        for field in data:
            ...
            try:
                ...
                print(f"WARNING unkown header type received: {hex(field_data[self.type_ind][1])} (while parsing {self.name})")
                return
    ...
    def unpack(self, data):
        ...
        while ind < self.length:
            if ind > self.length - self.type_len:
                #print("WARNING, short trailing data", self.length, len(data), data[ind:])
                return (self.name, ret), ind
            ...
            try:
                ...
                print(f"WARNING unkown header type received: {hex(hdr_type)} (while parsing {self.name})")
                print(f"Data: {data[ind:][:0x20].hex()}")
                return (self.name, ret), ind
...
class RepeatedFieldHeader(Header):
    def __init__(self, name, fields, ind_count, ind_repeated, padding_field=None):
        super().__init__(name, fields)
        self.ind_count = ind_count
        self.ind_repeated = ind_repeated
    ...
    def pack(self, args):
        ...
        while arg_ind < len(args):
            if field_ind == self.ind_count:
                repetitions = args[arg_ind]
            if field_ind == self.ind_repeated:
        ...
        for field_ind, field in enumerate(self.fields):
            ...
            if field_ind in self.indicator_map:
                ...
                try:
                    ...
                    print(f"WARNING: Unknown type {type_f}!")
                    print(f"Data: {data[ind:][:0x20].hex()}")
            else:
                value, length = field.unpack(data[ind:])
...
class BERField(Field):
    def __init__(self, name, fmt, datatype, print_fmt=None, dynamic_fmt=False):
...
class BERSequence(BERField):
    def __init__(self, name, datatype, fields):
